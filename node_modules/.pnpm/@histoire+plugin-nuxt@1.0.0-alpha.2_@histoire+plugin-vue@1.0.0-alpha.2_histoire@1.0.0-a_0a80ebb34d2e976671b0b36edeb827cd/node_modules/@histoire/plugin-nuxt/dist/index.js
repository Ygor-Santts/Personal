...
export function HstNuxt() {
    ...
    return {
        ...
        async defaultConfig() {
            ...
            const plugins = viteConfig.plugins.filter((p) => !ignorePlugins.includes(p?.name));
            return {
                vite: {
                    server: {
...
export async function setupVue3 () {
  await setupNuxtApp(${JSON.stringify(nuxt.options.runtimeConfig.public)})
                ...
                viteNodeInlineDeps: [
                    ...
                    /^#/,
                    /\.nuxt/,
                    ...nuxt.options.build.transpile.filter(r => typeof r === 'string' || r instanceof RegExp),
                ],
                build: {
...
async function useNuxtViteConfig() {
    ...
    nuxt.options.build.templates.push({ src: join(runtimeDir, 'composables.mjs'), filename: 'histoire/composables.mjs' }, { src: join(runtimeDir, 'components.mjs'), filename: 'histoire/components.mjs' });
