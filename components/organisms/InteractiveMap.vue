  ...
  >
    ...
    >
      ...
      <div
        ...
        :title="
          props.editorMode
            ? `Hotspot ${hotspot.number} - Right click to remove`
            : hotspot.title
        "
      >
      ...
      <!-- Tooltip inside map to follow transformations -->
      <Tooltip
    ...
    <!-- Editor instructions (only in editor mode) -->
    <div
    ...
    >
      <div class="font-semibold mb-1">Hotspot Editor Mode</div>
      <div class="text-xs opacity-90">
        • Click on map to add<br />
        • Right click on hotspot to remove
      </div>
    </div>

    <!-- Export button (only in editor mode with tools) -->
    <div
      v-if="props.editorMode && props.showEditorTools"
    ...
    >
      <button
        ...
        aria-label="Export hotspots"
      >
        Export ({{ hotspotsData.length }})
      </button>
      <button
        @click="clearHotspots"
    ...
    >
      <p class="text-gray-700 mb-4">
        Error loading map. Please check if the file exists.
      </p>
      <button
        @click="retryLoad"
...
const tooltipPosition = ref({ x: 50, y: 50 });
...
const minScale = ref(1); // Dynamic minimum scale (fills container)
...
const hasMoved = ref(false); // To detect if moved during click
...
const handleMapClick = (e: MouseEvent) => {
  // Only add hotspot in editor mode
  if (!props.editorMode) return;

  // Don't add hotspot if there was movement (pan)
  ...
  // Calculate coordinates as percentage (0-100)
  ...
  const newHotspot: HotspotData = {
    ...
    title: `Hotspot ${nextHotspotNumber.value}`,
    description: `Description of hotspot ${nextHotspotNumber.value}`,
    x: Math.round(percentX * 100) / 100, // 2 decimal places
...
const handleHotspotClick = (hotspot: HotspotData) => {
  ...
  tooltipPosition.value = { x: hotspot.x, y: hotspot.y };
...
const exportHotspots = () => {
  ...
  if (navigator.clipboard) {
    navigator.clipboard
      .writeText(data)
      .then(() => {
        alert(`${hotspotsData.value.length} hotspots copied to clipboard!`);
      })
      .catch(() => {
        alert(
          `Data displayed in console. Total: ${hotspotsData.value.length} hotspots`
        );
      });
  } else {
    alert(
...
const clearHotspots = () => {
  if (confirm("Do you really want to clear all hotspots?")) {
    hotspotsData.value = [];
...
// Function to apply pan limits (don't drag outside the map)
const applyPanLimits = (x: number, y: number): { x: number; y: number } => {
  ...
  // Calculate limits
  // The map should not be dragged beyond container limits
  ...
  // If map is smaller than container in any dimension, center it
...
const resetView = () => {
  if (containerRef.value && imageWidth.value > 0) {
    ...
    // Recalculate scale to fill container
...
const onPan = (e: MouseEvent | TouchEvent) => {
  ...
  // Apply limits to prevent dragging outside the map
...
const onImageLoad = (event: Event) => {
  ...
  if (containerRef.value && event.target) {
    ...
    // Calculate scale to fill container maintaining aspect ratio
    const scaleX = containerWidth.value / naturalWidth;
    const scaleY = containerHeight.value / naturalHeight;
    const fillScale = Math.max(scaleX, scaleY); // Use the largest to fill completely

    // Set minimum scale as the scale that fills the container
    minScale.value = fillScale;
...
// Close tooltip with ESC
const handleKeydown = (event: KeyboardEvent) => {
  if (event.key === "Escape" && isTooltipVisible.value) {
    closeTooltip();
  }
};
...
const handleResize = () => {
  if (containerRef.value && imageWidth.value > 0) {
    ...
    // Recalculate scale to maintain fill
    const scaleX = containerWidth.value / imageWidth.value;
    const scaleY = containerHeight.value / imageHeight.value;
    const fillScale = Math.max(scaleX, scaleY);
    ...
    // If current scale is less than new minimum scale, adjust it
