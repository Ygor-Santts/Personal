...
const centerIndex = computed(() => {
  ...
  return props.clonesPerSide + currentIndex.value;
});
...
const getSlideOpacity = (index: number) => {
  ...
  // Calcular opacidade (fade out com distância)
  return Math.max(0.3, Math.min(1, 1 - (distance / maxDistance) * 0.7));
};
...
const getSlideTransform = (index: number) => {
  ...
  // Calcular escala (como no Swiper: 0.6 a 1.2)
  const scale = Math.max(
    0.6,
    Math.min(1.2, 1 - (Math.abs(distance) / maxDistance) * 0.4)
  );
